...
$(stamp)install_%: $(stamp)check_%
	...
	if [ $(curpass) = libc ]; then \
	  ...
	  #MA# mkdir -p debian/tmp-$(curpass)/usr/include; \
	  #MA# mv debian/tmp-$(curpass)/usr/include/bits debian/tmp-$(curpass)/usr/include; \
	  #MA# mv debian/tmp-$(curpass)/usr/include/gnu debian/tmp-$(curpass)/usr/include; \
	  #MA# mv debian/tmp-$(curpass)/usr/include/sys debian/tmp-$(curpass)/usr/include; \
	  #MA# mv debian/tmp-$(curpass)/usr/include/fpu_control.h debian/tmp-$(curpass)/usr/include; \
	  #MA# mv debian/tmp-$(curpass)/usr/include/a.out.h debian/tmp-$(curpass)/usr/include; \
	  #MA# mv debian/tmp-$(curpass)/usr/include/ieee754.h debian/tmp-$(curpass)/usr/include; \
	fi
	...
	if [ $(curpass) = libc ]; then \
	  ...
	  if ! test -f $(target); then ln -s $$target $$link_name ; fi ; \
	fi
