...
$(stamp)install_libc: $(stamp)check_libc
	...
	for lib_a in $(stage1_libfake.a); do \
		install -T $(DEB_BUILDDIR)/libfake.a $(DESTDIR)/usr/lib/$$lib_a; \
	done
	for lib_so in $(stage1_libfake.so); do \
		install -T $(DEB_BUILDDIR)/libfake.so $(DESTDIR)/usr/lib/$$lib_so; \
	done
	$(MAKE) -C $(DEB_BUILDDIR) install_root=$(DESTDIR) install-bootstrap-headers=yes \
		csu/subdir_install install-headers
	mkdir -p $(DESTDIR)/usr/include/
	#MA# mv $(DESTDIR)/usr/include/bits          $(DESTDIR)/usr/include/$(DEB_HOST_MULTIARCH)
	#MA# mv $(DESTDIR)/usr/include/gnu           $(DESTDIR)/usr/include/$(DEB_HOST_MULTIARCH)
	#MA# mv $(DESTDIR)/usr/include/sys           $(DESTDIR)/usr/include/$(DEB_HOST_MULTIARCH)
	#MA# mv $(DESTDIR)/usr/include/fpu_control.h $(DESTDIR)/usr/include/$(DEB_HOST_MULTIARCH)
	#MA# mv $(DESTDIR)/usr/include/a.out.h	$(DESTDIR)/usr/include/$(DEB_HOST_MULTIARCH)
	#MA# mv $(DESTDIR)/usr/include/ieee754.h	$(DESTDIR)/usr/include/$(DEB_HOST_MULTIARCH)
