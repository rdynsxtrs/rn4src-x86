...
dirs_gcc = \
	$(docdir)/$(p_base)/{gcc,libssp,gomp,itm,quadmath} \
	$(PF)/bin \
	$(gcc_lexec_dir) \
...
ifeq ($(with_itm),yes)
	...
	cp -p $(srcdir)/libitm/ChangeLog \
		$(d_gcc)/$(docdir)/$(p_base)/itm/changelog
endif
ifeq ($(with_qmath),yes)
...
$(binary_stamp)-gcc-multi: $(install_dependencies)
	...
	mkdir -p $(d_gcc_m)/usr/share/lintian/overrides
	cp -p debian/$(p_gcc_m).overrides \
		$(d_gcc_m)/usr/share/lintian/overrides/$(p_gcc_m)
